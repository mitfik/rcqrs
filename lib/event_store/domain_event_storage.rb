module EventStore
  ...
  def EventStore.create
    case Rcqrs::Setting.default_orm 
      when :data_mapper
      ...
      when :active_record
        ...
        config = YAML.load_file(File.join(Rails.root, Rcqrs::Setting.default_database_file_path))[Rails.env]
      ...
      when :in_memory
        ...
        raise "This ORM is not supported yet: #{Rcqrs::Setting.default_orm}, try use :data_mapper, :active_record or :in_memory"
    end
  end
end
